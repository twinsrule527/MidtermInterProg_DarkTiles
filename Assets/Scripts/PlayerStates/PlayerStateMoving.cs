...
public class PlayerStateMoving : PlayerState
{
    ...
    //This state takes half the amount of time the other actions take
    public override void Run()
    {
        curTime += Time.deltaTime;
        //After the max time, it reaches the end
        if(curTime > PlayerControl.TIMEFORACTION / 2f) curTime = PlayerControl.TIMEFORACTION / 2f;

        PlayerControl.Instance.transform.position = Vector3.Lerp(startPos, target, curTime / (PlayerControl.TIMEFORACTION / 2f));
        ...
        if(curTime == PlayerControl.TIMEFORACTION / 2f) {
